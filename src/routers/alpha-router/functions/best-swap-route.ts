...
export async function getBestSwapRouteBy(
  ...
  let quoteGasAdjusted = sumFn(
    _.map(bestSwap, (routeWithValidQuote) => routeWithValidQuote.quoteAdjustedForGas)
  );
    ...
    if (v2GasModel == undefined && v3GasModel == undefined) {
      ...
      if (v2Routes.length > 0 && V2_SUPPORTED.includes(chainId)) {
        if (v2GasModel) {
          ...
          gasCostsL1ToL2.gasCostL1QuoteToken = gasCostsL1ToL2.gasCostL1QuoteToken.add(
  ...
  if (routingConfig.gasToken) {
    ...
    if (
      bestSwap.some((routeWithValidQuote) => routeWithValidQuote.gasCostInGasToken === undefined)
    ) {
      log.info(
        {
          bestSwap,
    ...
    estimatedGasUsedGasToken = sumFn(
      _.map(
        ...
        (routeWithValidQuote) => routeWithValidQuote.gasCostInGasToken as CurrencyAmount
      )
    );
  }
