...
> {
  ...
  public getRoutesThenQuotes(
    ...
    return this.getRoutes(tokenIn, tokenOut, candidatePools, tradeType, routingConfig).then(
      (routesResult) => {
  ...
  protected async applyTokenValidatorToPools<T extends V3Pool | Pair>(
    pools: T[],
    ...
    const tokenValidationResults = await this.tokenValidatorProvider.validateTokens(
